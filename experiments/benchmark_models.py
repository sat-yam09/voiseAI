"""Benchmark three multilingual embedding models on hi/en/bn/gu queries.

Measures Recall@5, Hit Rate@5, embedding latency, FAISS search latency,
and model/index sizes.  Skips the cross-encoder reranker.

Passage embeddings are cached per-model in separate directories so
re-runs skip encoding entirely.
...
TEST_QUERIES = [
    ...
    # Bengali (Assamese proxy — Bengali script)
    ...
    {"qid": "116095",  "lang": "gu", "query": "\u0aae\u0ac3\u0a24\u0acd\u0a2f\u0ac1 \u0aa6\u0a23\u0acd\u0aa1 \u0aa8\u0abe \u0ab5\u0acd\u0a2f\u0abe\u0a16\u0acd\u0a2f\u0abe\u0a28"},
...
def compute_recall_hit(relevant_ids, retrieved_ids, k=5):
...
def _load_or_encode_passages(embedder, chunks, model_name):
    """Encode passages or load from a per-model numpy cache."""
    cache_dir = os.path.join(PROJECT_ROOT, "data", "cache", "bench_" + model_name.replace("/", "_"))
    os.makedirs(cache_dir, exist_ok=True)
    cache_path = os.path.join(cache_dir, "passages.npy")

    if os.path.exists(cache_path):
        vecs = np.load(cache_path)
        print("       Loaded cached passages: shape={} from {}".format(vecs.shape, cache_path))
        return vecs

    print("       No cache found. Encoding from scratch ...")
    t0 = time.perf_counter()
    vecs = embedder.encode_chunks(chunks, show_progress=True)
    enc_ms = (time.perf_counter() - t0) * 1000
    print("       shape={}  encode_time={:.0f}ms ({:.1f}s)".format(
        vecs.shape, enc_ms, enc_ms / 1000))
...
def main():
    ...
    for model_cfg in MODELS:
        ...
        # ── 1. Encode or load passages ──
        print("\n  [1] Passages ...")
        vecs = _load_or_encode_passages(embedder, chunks, model_name)
        ...
        print("  [3] Model size (HF cache): {:.1f} MB".format(model_size_mb))
        print("       Index size on disk:   {:.1f} MB".format(index_size_mb))

        # ── 4. Query evaluation ──
        print("  [4] Evaluating queries ...")
        lang_results = {}
        all_embed_lats = []
        all_search_lats = []
        all_total_lats = []
        ...
        for lang in ["en", "bn", "hi", "gu"]:
            ...
            for q in qlist:
                ...
                r, h = compute_recall_hit(relevant_ids, ret_ids, k=5)
        ...
        agg_recall = sum(r["recall@5"] * r["count"] for r in lang_results.values()) / max(sum(r["count"] for r in lang_results.values()), 1)
        agg_hit = sum(r["hit_rate@5"] * r["count"] for r in lang_results.values()) / max(sum(r["count"] for r in lang_results.values()), 1)
        ...
        target_met = "YES" if agg_total_p50 <= 200 else "NO"

        print("\n  AGGREGATE: Recall@5={:.4f}  Hit@5={:.4f}".format(agg_recall, agg_hit))
        print("  Total retrieval (embed+search) p50={:.1f}ms  p95={:.1f}ms  <=200ms? {}".format(
            agg_total_p50, agg_total_p95, target_met))
    ...
    print("=" * 72)
    print("{:<42s} {:>10s} {:>8s} {:>8s} {:>8s} {:>10s} {:>8s}".format(
        "Model", "Recall@5", "Hit@5", "Emb p50", "Srch p50", "Total p50", "<=200ms?"))
    print("  " + "-" * 100)
    for name, res in results_all.items():
        a = res["aggregate"]
        print("{:<42s} {:>10.4f} {:>8.4f} {:>6.0f}ms {:>6.0f}ms {:>8.0f}ms {:>8s}".format(
            name, a["recall@5"], a["hit_rate@5"],
            a["embed_p50_ms"], a["search_p50_ms"], a["total_p50_ms"],
    ...
    print("  {:<42s} {:>8s} {:>10s}".format("Model", "Model MB", "Index MB"))
    print("  " + "-" * 64)
    for name, res in results_all.items():
        print("  {:<42s} {:>8.1f} {:>10.1f}".format(
            name, res["model_size_mb"], res["index_size_mb"]))
    print()
    ...
    best = max(results_all.items(), key=lambda kv: kv[1]["aggregate"]["recall@5"])
    best_name = best[0]
    best_res = best[1]
    under_200 = best_res["aggregate"]["under_200ms"]
    print("RECOMMENDATION: {} (Recall@5={:.4f}, total_p50={:.0f}ms, <=200ms={})".format(
        best_name, best_res["aggregate"]["recall@5"],
        best_res["aggregate"]["total_p50_ms"],
        "YES" if under_200 else "NO"))
